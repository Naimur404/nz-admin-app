...
export default function HotelBookingDetailsScreen() {
  ...
  const loadBookingDetails = async () => {
    ...
    try {
      ...
      loadOperationLogs(transactionId as string);
    } catch (error: any) {
  ...
  const loadOperationLogs = async (transId: string) => {
    try {
      setLogsLoading(true);
      const response = await hotelService.getBookingOperationLog(transId);
      setOperationLogs(response.data || []);
      console.log('Hotel operation logs loaded successfully');
    } catch (error: any) {
